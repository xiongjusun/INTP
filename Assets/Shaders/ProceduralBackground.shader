Shader "Unlit/ProceduralBackground"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float _NoiseScale1, _NoiseScale2, _NoiseScale3;
            int _BlendMode;
            ...
            float voronoi(float2 st)
            {
                ...
                for (int y = -1; y <= 1; y++)
                {
                    for (int x = -1; x <= 1; x++)
                    {
                        ...
                        randP = 0.5 + 0.5 * sin(_Time.y * 0.5 + 6.2831 * randP);
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                float3 noises = layeredNoise(uv, _Time.y);
                ...
                float4 color = applyGradient(finalNoise);
